...
NSW-compatible point format. This complements the existing tmr.geojson
hourly-profile dataset — it is the raw census AADT site layer.
...
def build() -> None:
    ...
    for row in rows:
        ...
        lat = parse_number(first_prop(row, "site_latitude", "latitude", "lat", "gps_latitude", default=None))
        lon = parse_number(first_prop(row, "site_longitude", "longitude", "lon", "long", "gps_longitude", default=None))
        ...
        if not within_bounds(lat, lon, QLD_BOUNDS):
            skipped += 1
            continue
        site = str(first_prop(row, "site", "site_id", "site_number",
        ...
        year_num = parse_number(first_prop(row, "traffic_year", "year", "census_year", "yr",
                                           "count_year", default=None))
        ...
        # (the census lists AGAINST GAZETTAL / BOTH DIRECTIONS / WITH GAZETTAL
        # per site/year; the directional row HALVES the AADT). Selection key =
        # (year, direction priority): newest year wins, ties prefer BOTH.
        ...
        if existing and existing["select_key"] >= select_key:
            continue
        # TMR's class scheme is hierarchical: AADT = CLASS_0A (light, Austroads
        # classes 1-2) + CLASS_0B (heavy, classes 3-12). PC_CLASS_0B is the
        # heavy-vehicle share; AADT_CLASS_0B the heavy volume. (Do NOT sum the
        # 2A-2L bins — they re-split the whole population and total ~100%.)
        ...
        best_by_site[site] = {
            ...
            "district": str(first_prop(row, "district_name", "region_name", "district", "region", "tmr_district")).strip().title(),
        }
