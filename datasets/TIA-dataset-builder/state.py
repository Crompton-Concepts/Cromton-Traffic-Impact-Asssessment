...
class SourceUnavailable(RuntimeError):
    """Raised when a state has no usable public data source (as opposed to a
    transient build failure). build_all_states reports these as SKIPPED."""
...
# National Freight Data Hub "Harmonised Traffic Counts" (HTC_monthly layer).
# NOTE: the layer publishes MONTHLY station totals in classification-bin
# fields (bins01_/bins02_/bins04_/bins12_*) — there is no AADT field — and as
# of June 2026 it only contains NSW, TAS and VIC stations.
NFDH_LAYER = ("https://spatial.infrastructure.gov.au/server/rest/services/"
              "Hosted/Harmonised_Traffic_Counts/FeatureServer/0")
_NFDH_BIN_GROUPS = (
    ("bins01_class01to12",),
    ("bins02_class01to02", "bins02_class03to12"),
    ("bins04_class01to02", "bins04_class03to05", "bins04_class06to09",
     "bins04_class10to12"),
    tuple(f"bins12_class{i:02d}" for i in range(1, 13)),
)


def _nfdh_monthly_total(props: dict) -> float | None:
    """Best monthly vehicle total from the classification-bin fields."""
    for group in _NFDH_BIN_GROUPS:
        vals = [parse_number(props.get(name)) for name in group]
...
def nfdh_monthly_features(state_code: str, bounds: tuple, prefix: str,
                          layer_url: str = NFDH_LAYER) -> list[dict]:
    """Aggregate NFDH monthly totals into per-station AADT point features
    (latest year with data per station)."""
    import re
    feats = arcgis_query_all(layer_url, where=f"state = '{state_code}'")
    print(f"  NFDH rows for {state_code}: {len(feats):,}")

    # station -> {"meta": {...}, "periods": {(year, period): [veh_sum, days]}}
    stations: dict[str, dict] = {}
    for feat in feats:
        props = feat.get("properties") or {}
        total = _nfdh_monthly_total(props)
        if not total:
            continue
        lon, lat = midpoint_from_geometry(feat.get("geometry"))
        if lon is None or lat is None or not within_bounds(lat, lon, bounds):
            continue
        period = str(props.get("temporal_period") or "").strip()
        year_match = re.search(r"(19|20)\d{2}", period)
        if year_match:
            year = int(year_match.group(0))
        else:
            t_from = parse_number(props.get("temporal_coverage_from"))
            if t_from is None:
                continue
            year = datetime.fromtimestamp(t_from / 1000, tz=timezone.utc).year
        t_from = parse_number(props.get("temporal_coverage_from"))
        t_to = parse_number(props.get("temporal_coverage_to"))
        days = 30.0
        if t_from is not None and t_to is not None and t_to > t_from:
            days = min(max((t_to - t_from) / 86_400_000.0, 1.0), 366.0)

        site_id = str(props.get("src_stnid") or props.get("src_stnname")
                      or f"{round(lat,4)}_{round(lon,4)}").strip()
        st = stations.setdefault(site_id, {
            "meta": {"lon": lon, "lat": lat,
                     "road": str(props.get("roadname") or "").strip()},
            "periods": {},
        })
        rec = st["periods"].setdefault((year, period or str(year)), [0.0, days])
        rec[0] += total  # directional rows for the same month sum together;
        # days counted once per period.

    output = []
    for site_id, st in stations.items():
        by_year: dict[int, list[float]] = {}
        for (year, _period), (veh, days) in st["periods"].items():
            agg = by_year.setdefault(year, [0.0, 0.0])
            agg[0] += veh
            agg[1] += days
        best = None
        for year in sorted(by_year, reverse=True):
            veh, days = by_year[year]
            if days > 0 and veh / days >= 1:
                best = (year, veh / days)
                break
        if not best:
            continue
        ...
        output.append(make_point_feature(
            ...
            station_id=site_id,
            road_name=(meta["road"] or f"{prefix} Road").title(),
            lon=meta["lon"], lat=meta["lat"],
            traffic_count=adt,
            year=str(year),
            road_hierarchy=f"{prefix} Road",
        ))
    return output
