...
def wa_year(value) -> str:
    """Handle plain years and financial-year strings like '2023/24'."""
    import re
    match = re.search(r"(19|20)\d{2}", str(value or ""))
    return safe_year(match.group(0) if match else None)
...
def build() -> None:
    ...
    for feat in features:
        ...
        aadt = parse_number(first_prop(
            props, "AADT", "ADT", "AAWDT", "AVE_VEH", "AVERAGE_VEHICLES",
            "MON_FRI_AVE", "VOLUME", "TOTAL_VEHICLES",
            # Traffic Digest layer: MON_SUN is the 7-day average daily volume
            # (AADT equivalent); MON_FRI is the weekday average fallback.
            "MON_SUN", "MON_FRI", default=None))
        ...
        hv = parse_number(first_prop(props, "PCT_HV", "HV_PERCENT", "PER_HEAVY",
                                     "HEAVY_PCT", "PERCENT_HEAVY",
                                     "PCT_HEAVY_MON_SUN", "PCT_HEAVY_MON_FRI", default=None))
        ...
        feature = make_point_feature(
            ...
            lon=lon, lat=lat,
            traffic_count=aadt,
            year=wa_year(first_prop(props, "YEAR", "COUNT_YEAR", "DATA_YEAR",
                                    "TRAFFIC_YEAR", default=None)),
            suburb=str(first_prop(props, "SUBURB", "LOCALITY")).strip().title(),
            lga=str(first_prop(props, "LGA_NAME", "LGA", "LG_NAME")).strip().title(),
            region=str(first_prop(props, "REGION", "MRWA_REGION", "RA_NAME")).strip().title(),
